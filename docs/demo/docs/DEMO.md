# EDRP Browser Demo (Screenshots)
## Evidence-based Mathematical Decision Engine / 증거기반 수학적 판정 엔진

**EN**  
This demo shows the *evidence → decision → label → tracking* workflow using GitHub only (no secret logic exposed).

**KR**  
이 데모는 GitHub만으로 **증거 → 판정 → 라벨 → 추적** 흐름이 자동으로 도는 모습을 보여줍니다(비밀 로직 노출 없음).

---

## Step 1 — Submit a PoC request (structured intake) / PoC 요청 제출(구조화 접수)
...
## Step 2 — Auto labels + quick evidence check / 자동 라벨링 + 형식 검사
...
## Step 3 — Maintainer forwarding / 유지보수자 포워딩
...
## Step 4 — Internal ticket created in edict-kit / 내부 티켓 자동 생성
...
## Step 5 — Tracked in EDRP Leads board / 보드에서 자동 추적
